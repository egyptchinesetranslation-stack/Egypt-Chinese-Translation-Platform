...
import { Search, Send, CheckCircle2, Paperclip, Image, Smile, ArrowLeft } from "lucide-react";
...
const text = {
  en: {
    ...
    noChatSub: "Choose a contact from the list to start chatting",
...
function Dashboard() {
  ...
  // Auto-open a chat from URL param ?chat=translatorId
  useEffect(() => {
  ...
  // Auto-create chat docs for all booking translators (so every job partner shows up)
  useEffect(() => {
    ...
    (async () => {
      ...
      const translatorIds = [...new Set(bSnap.docs.map((d) => d.data().translatorId))];
      await Promise.all(
        translatorIds.map(async (tid) => {
          const chatId = getChatId(currentUser.uid, tid);
          const chatRef = doc(db, "chats", chatId);
          await setDoc(chatRef, {
  ...
  return (
    <div className="db-page" style={{ backgroundImage: `url(${backgroundImage})` }}>
      {/* LEFT SIDEBAR */}
      <aside className={`db-sidebar ${showChat ? "db-sidebar--hidden" : ""}`}>
        <h2 className="db-sidebar__title">{t.messages}</h2>
        ...
        <ul className="db-user-list">
          ...
          {filteredChats.map((chat) => {
            ...
            return (
              <li
                key={chat.id}
                className={`db-user-item ${activeChat === chat.id ? "db-user-item--active" : ""}`}
                onClick={() => { setActiveChat(chat.id); setShowChat(true); }}
              >
                <div className="db-user-item__avatar-wrap">
                  <img
                    className="db-user-item__avatar"
                    src={other.photoURL || getAvatarUrl(other.name)}
                    alt={other.name || "User"}
                  />
                </div>
                <div className="db-user-item__info">
                  <div className="db-user-item__row">
                    <span className="db-user-item__name">{other.name || "User"}</span>
                    <span className="db-user-item__time">
                      {chat.lastMessageTime ? formatTime(chat.lastMessageTime) : ""}
                    </span>
                  </div>
                  <p className="db-user-item__msg">{chat.lastMessage || ""}</p>
                </div>
              </li>
            );
          })}
      ...
      {/* CENTER CHAT */}
      <main className={`db-chat ${!showChat ? "db-chat--hidden" : ""}`}>
        {activeOtherUser ? (
          <>
            {/* Chat Header */}
            <div className="db-chat__header">
              <div className="db-chat__header-left">
                <button className="db-chat__back-btn" onClick={() => setShowChat(false)} aria-label="Back">
                  <ArrowLeft size={20} />
                </button>
                <div className="db-chat__header-avatar-wrap">
                  <img
                    className="db-chat__header-avatar"
                    src={activeOtherUser.photoURL || getAvatarUrl(activeOtherUser.name)}
                    alt={activeOtherUser.name}
                  />
                </div>
                <div>
                  <div className="db-chat__header-name">
                    {activeOtherUser.name}
                    <CheckCircle2 size={14} className="db-chat__verified" />
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="db-chat__messages">
              {chatMessages.map((msg, idx) => {
                ...
                return (
                  <div key={msg.id}>
                    {dateSep && (
                      <div className="db-chat__separator">
                        <span>{dateSep}</span>
                      </div>
                    )}
                    <div className={`db-chat__bubble-wrap ${isMe ? "db-chat__bubble-wrap--right" : ""}`}>
                      <div className={`db-chat__bubble ${isMe ? "db-chat__bubble--me" : "db-chat__bubble--them"}`}>
                        {msg.text && <p className="db-chat__bubble-text">{msg.text}</p>}
                      </div>
                      <span className={`db-chat__bubble-time ${isMe ? "db-chat__bubble-time--right" : ""}`}>
            ...
            {/* Input */}
            <div className="db-chat__input-bar">
              <input
